...
// TODO: YOUR "branched" out-off-place sort implementation
int sort_branched_outoffplace( int N , int *input_array , int *result_array ) {

	// TODO:
	// replace this with your
	// "branched" out-off-place sort implementation
...
int main( int argc , char ** argv ) {
	...
	gettimeofday( &before , NULL );
	int x = sort_branched_outoffplace( N , input_array , result_array );
	gettimeofday( &after , NULL );
	...
	if ( x != 0 ) {
		...
		fprintf(
			stderr ,
			"sort_branched_outoffplace failed\n"
		);
		// free allocated memory
		free( result_array );
		free( input_array );
