...
// TODO: YOUR "predicated" out-off-place sort implementation
int sort_predicated_outoffplace( int N , int *input_array , int *result_array ) {

	// TODO:
	// replace this with your
	// "predicated" (branch-free) out-off-place sort implementation
...
int main( int argc , char ** argv ) {
	...
	if ( input_array == NULL ) {
		...
		fprintf(
			stderr ,
			"malloc failed for input_array (%zd bytes)\n" ,
			N * sizeof(int)
		);
	...
	if ( result_array == NULL ) {
		...
		fprintf(
			stderr ,
			"malloc failed for result_array (%zd bytes)\n" ,
			N * sizeof(int)
		);
		// free allocated memory
		free( input_array );
	...
	// sort input array out-off-place (into result array);
	// measure the time it takes
	gettimeofday( &before , NULL );
	int x = sort_predicated_outoffplace( N , input_array , result_array );
	gettimeofday( &after , NULL );
	...
	if ( x != 0 ) {
		...
		fprintf(
			stderr ,
			"sort_predicated_outoffplace failed\n"
		);
		// free allocated memory
		free( result_array );
		free( input_array );
