...
// Bubble sort without branching
int sort_predicated_inplace( int N , int *data_array ) {

	for ( int i = 0 ; i < N ; i++ ) {
		for ( int j = 0; j < N - i - 1 ; j++){
			int x = data_array[j];
			int y = data_array[j+1];
			data_array[j] -= (x > y) * (x - y);
            data_array[j+1] += (x > y) * (x - y);
		}
	}
...
int main( int argc , char ** argv ) {
	...
	if ( data_array == NULL ) {
		...
		fprintf(
			stderr ,
			"malloc failed for data_array (%zd bytes)\n" ,
			N * sizeof(int)
		);
	...
	gettimeofday( &before , NULL );
	int x = sort_predicated_inplace( N , data_array );
	gettimeofday( &after , NULL );
	...
	if ( x != 0 ) {
		...
		fprintf(
			stderr ,
			"sort_predicated_inplace failed\n"
		);
		// free allocated memory
		free( data_array );
		free( input_data_file_location );
